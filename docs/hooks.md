...
**What a handler can expect from the dispatcher** (`openspec/specs/hook-handler-dispatch/spec.md`): it
runs exactly once per dispatch, and an error it raises reaches the code that fired the hook unchanged,
so a handler that fails after a side effect never has that side effect repeated. Its plugin's helper
modules are included on demand before it runs; a handler that none of them defines is skipped with a
logged warning, on the controller and the view dispatch paths alike.
