...
- **Bug fix:** A hash meta, such as a record's options, now reads back on the instance that wrote it with indifferent keys, as a fresh load returns it. A String-keyed `set_option` after Symbol-keyed options (a new post type's defaults) replaces the value instead of storing the key twice, which json gem 3 refuses to generate. [#1292](https://github.com/owen2345/camaleon-cms/pull/1292).
