...
- **Security fix:** Fix HTML injection in `Hash#to_attr_format` and in the bundled `cama_contact_form` plugin's form rendering. `to_attr_format` escaped attribute values with `gsub('"', '\"')` — a Ruby string escape applied to an HTML context, where a backslash escapes nothing — so a value of `x" onfocus=alert(1) y="` rendered as a live `onfocus` attribute. It also interpolated attribute *names* verbatim, so a key of `x onfocus=alert(1) y` rendered as three attributes; escaping cannot defend that position, since the splitting characters are whitespace and `=` rather than HTML metacharacters, so keys are now validated and invalid pairs dropped. Values are escaped with `CGI.escapeHTML` rather than `ERB::Util.html_escape`, which is a no-op on `html_safe` strings and would have left such a value able to break out. Separately, `cama_form_element_bootstrap_object` built its markup by raw string interpolation with no escaping at any position and the shortcode emitted the result through `raw`, so both an unauthenticated visitor's resubmitted values (stashed in `flash[:values]` when validation fails) and every form-definition field — label, default value, option labels, CSS class — could inject script. Because the admin panel is same-origin with the frontend, script landing there runs with an administrator's session, making this a privilege escalation from any role holding `:manage, :plugins`. Requires `cama_contact_form` 0.1.10. [#PR](https://github.com/owen2345/camaleon-cms/pull/PR) — thanks, Amir Aliu and Enrik Mustafa, for reporting this.
  ...
  - **Behavior change:** the role editor gains an "Allow unfiltered HTML in contact forms" permission under Manager Permissions, off for every role except `admin`. Without it, a contact form's `previous_html`, `after_html` and field templates are sanitized when saved. See `docs/security/permissions.md`.
  ...
  - **Upgrade note:** HTML typed into a contact form's field labels, default values, option labels or CSS class now renders as visible text rather than markup. These are plain-text inputs in the form editor, but nothing enforced it before.

- **Security bumps:** concurrent-ruby 1.3.7 → 1.3.8, erb 6.0.4 → 6.0.6, json 2.20.0 → 2.21.1, and net-imap 0.6.4.1 → 0.6.6, picked up while re-resolving `cama_contact_form`. [#PR](https://github.com/owen2345/camaleon-cms/pull/PR)
