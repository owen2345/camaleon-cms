...
- **Bug fix:** On the record object that wrote its options with `set_meta`, `options` and `get_option` read an option by a String key or its Symbol twin, as a freshly loaded record does, and options that are nil or an empty string read as empty and accept writes instead of raising. [#1302](https://github.com/owen2345/camaleon-cms/pull/1302).
