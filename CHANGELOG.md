...
- **Security fix:** Fix stored XSS via unsanitized post content — add server-side HTML sanitization to `Post#content` at save time with role-based allowlisting (new `allow_unfiltered_html` permission key), so untrusted contributors cannot inject arbitrary scripts into `raw @post.the_content` template output. Also fix data loss from `normalize_attrs` on plain-text and structured-data fields (Meta#value, User#name, Site#name, etc.) where `sanitize()` silently stripped angle-bracket content such as `email_from: "Name <user@domain.com>"`, [#1206](https://github.com/owen2345/camaleon-cms/pull/1206) — thanks, Theodosis Paidakis, for reporting this.
