  ...
  def create_initializer_file
    if behavior == :revoke
      ...
      if PluginRoutes.all_themes.include?(PluginRoutes.theme_info(get_theme_name))
        ...
        # helpers + controllers
        plugin_app = File.join($camaleon_engine_dir, "lib", "generators", "theme_template", "app_#{get_theme_name}")
        plugin_folder = File.join(plugin_app, "apps", "themes", get_theme_name)

        FileUtils.rm_r(plugin_app) if Dir.exist?(plugin_app)

        # tmp copy
        FileUtils.cp_r(File.join($camaleon_engine_dir, "lib", "generators", "theme_template", "app"), plugin_app)
