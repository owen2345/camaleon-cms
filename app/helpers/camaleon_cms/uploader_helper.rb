...
module CamaleonCms
  module UploaderHelper
    ...
    def cama_stage_data_uri(uploaded_io, args, tmp_path)
      return [nil, { error: cama_t('camaleon_cms.admin.media.name_required').to_s }] if params[:name].blank?
    ...
    # Returns an error hash when size exceeds maximum, nil otherwise. Kept separate
    # from RuntimeUploaderConcern's copy on purpose: `ct` runs the `on_translation`
    # hook so plugins can override the message, which a shared I18n.t call would drop.
    def cama_size_limit_error(size, maximum)
      return if maximum.blank? || maximum >= size

      max_size = number_to_human_size(maximum)
      { error: "#{ct('file_size_exceeded', default: 'File size exceeded')} (#{max_size})" }
    end
  end
end
