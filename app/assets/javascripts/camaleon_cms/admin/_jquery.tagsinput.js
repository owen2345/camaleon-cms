...
/*
	Vendored from npm jquery-tags-input@1.3.5 (readable upstream of the minified
	bundle this engine previously shipped) with these local changes:

	1. jQuery.trim() replaced with native String#trim() (jQuery 3 removed
	   jQuery.trim's raison d'etre; native trim exists everywhere).
	2. The jQuery Autocompleter / jQuery UI autocomplete branch is replaced by
	   an Awesomplete adapter that keeps the jQuery-UI source contract:
	...
	Refresh source: https://registry.npmjs.org/jquery-tags-input/-/jquery-tags-input-1.3.5.tgz
         ...
         $(this).attr('data-tagsinput-init', true);
			...
			if (settings.interactive) {
				...
				var autocomplete_enabled =
					settings.autocomplete_url != undefined ||
					(settings.autocomplete && settings.autocomplete.source != undefined);

				if (autocomplete_enabled) {
					// Upstream wired jQuery Autocompleter or jQuery UI autocomplete here.
					// Both are gone with jQuery 3; Awesomplete provides the dropdown while
					// the jQuery-UI source contract is preserved for downstream callers:
					// source may be an array, a function invoked as source({term}, response)
					// or a URL string fetched with a `term` parameter; minLength is honored
					// alongside Awesomplete's minChars.
					var aco = jQuery.extend({source: settings.autocomplete_url}, settings.autocomplete);
