module CustomFieldsRead extend ActiveSupport::Concern
  ...
  def get_field_object(slug)
    CustomField.where(parent_id: get_field_groups.pluck(:id), slug: slug).first || CustomField.where(slug: slug, parent_id: get_field_groups({include_parent: true})).first
  end
