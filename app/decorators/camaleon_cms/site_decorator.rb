...
class CamaleonCms::SiteDecorator < CamaleonCms::TermTaxonomyDecorator
  ...
  def draw_languages(list_class = "language_list list-inline pull-right", current_page = false, current_class = "current_l")
    ...
    lan.each do |lang|
      ...
      res << "<li class='#{ current_class if I18n.locale.to_s == lang.to_s}'> <a href='#{h.cama_url_to_fixed(current_page ? "url_for" : "cama_root_url", {locale: lang})}'>#{lang.to_s.upcase}</a> </li>"
    end
    res << "</ul>"
    res.join("")
  end
