...
CSRF protection. Rails applies that protection to non-GET verbs only, so the write is confined to POST
while the render stays available on GET.
## Requirements
### Requirement: The custom-fields list category write requires a non-GET verb

`Admin::Settings::CustomFieldsController#list` SHALL persist a post's categories (via
`update_categories`) only on a non-GET request. A GET SHALL render the field groups for the resolved
record without mutating it, so that a cross-site GET — which carries the SameSite=Lax auth cookie on a
top-level navigation but no CSRF token — cannot change or delete a post's category relationships. The
route SHALL accept POST so a legitimate, CSRF-verified caller can still perform the write.
