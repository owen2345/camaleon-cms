# custom-field-group-placement Specification

## Purpose

Define which custom field groups a given owner renders in the admin panel. A `CustomFieldGroup` carries two independent foreign keys — `parent_id` for tenancy (which site owns and administers it) and `object_class` + `objectid` for placement (which record's admin page displays it). Reads MUST resolve placement, not tenancy: a group meant for posts, categories, themes or menus must never surface on the site settings form, where its required fields would block submission and its values would be discarded on save.

Also covers how a site's placement is established and kept consistent — stamped on programmatic creation, pinned to the current site on submission, and repairable for rows persisted without a placement id.
