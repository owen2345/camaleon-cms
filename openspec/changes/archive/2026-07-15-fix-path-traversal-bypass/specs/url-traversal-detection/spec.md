...
The system SHALL detect `../` path traversal in URI paths when the `reject_path_traversal: true` option is passed to the URL validator. Detection SHALL use `Addressable::URI.normalized_path` comparison to handle URL-encoded traversal (`%2e%2e`).
...
- **THEN** the raw path `uri.path` (`/../etc/passwd`) differs from `uri.normalized_path` (`/etc/passwd`), and the validator returns an error
...
- **THEN** the normalized path comparison detects the traversal and the validator returns an error
...
- **THEN** the raw path matches the normalized path, and validation passes
