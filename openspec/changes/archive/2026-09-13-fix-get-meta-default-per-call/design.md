...
- A write committed in a savepoint is kept under the savepoint's own state until no transaction is open: the
  rollback of any transaction around the savepoint marks that state rolled back too, as it marks every state
  begun inside it. Rejected, keeping it under the state of the transaction open at the next read, write or
  save, so that the writes of the savepoints of one transaction shared one entry: a state names no transaction
  around it, and the one open then can be a later savepoint beside the committed one or a transaction begun
  after its own, whose rollback was taken for the write's, reading the key again and dropping the metas built
  for it, which no save then stored. The cost: a record written in many savepoints of one transaction keeps an
  entry per savepoint until the transaction completes, each checked before every read, write and save.
