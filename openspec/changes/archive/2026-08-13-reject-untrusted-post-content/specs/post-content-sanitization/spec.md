# post-content-sanitization (delta)

## RENAMED Requirements

The remedy changes from transformation to refusal (maintainer decision, 2026-08-13), so the
requirements named after sanitization are renamed. Declared here because the MODIFIED section below
already references the new names.

- FROM: `### Requirement: Untrusted users' post content is sanitized at save time`
- TO: `### Requirement: Untrusted users' dangerous post content is rejected at save time`

- FROM: `### Requirement: Trusted users' post content bypasses sanitization`
- TO: `### Requirement: Trusted users' post content bypasses the content gate`

- FROM: `### Requirement: Content sanitization does not apply when user context is absent`
- TO: `### Requirement: The content gate fails closed when user context is absent`
