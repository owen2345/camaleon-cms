...
- `CamaleonCms::Admin::MediaController#crop` will validate `params[:cp_img_path]` via `UserUrlValidator` when it is an HTTP/HTTPS URL, before delegating to `cama_tmp_upload`
...
- **Affected code**: `app/controllers/camaleon_cms/admin/media_controller.rb` (the `crop` action)
