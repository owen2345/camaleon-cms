...
- **[A widget writes a value for the user before the baseline]** → A value a script sets during the wait (the thumbnail picker, a date picker) is not a touch: the baseline absorbs it like any setup write, and neither the prompt nor the timer sees it. The user's own input and the editors' dirty state are what count.
