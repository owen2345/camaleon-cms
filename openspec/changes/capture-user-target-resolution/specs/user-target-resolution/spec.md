...
This matches how the action already reports a missing `password` parameter — a status plus a short plain-text body — because the endpoint is called by an admin modal form that renders the response body directly. The `404` status itself is unchanged: `ActiveRecord::RecordNotFound` already maps to `:not_found`. Only the body format changes.
...
#### Scenario: Parameter errors keep their existing format
