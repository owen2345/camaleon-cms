...
Separately, the same endpoint is inconsistent about how it reports failures. `updated_ajax` answers a missing parameter with `400` plus a short text body and a validation error with `422` plus a short text body, but lets a lookup that finds no record fall through to the framework's default HTML error page. That inconsistency is folded into this change because it lives in the one method whose resolution behavior is being specified here.

A correction to an earlier draft of this document: that draft justified the fix by saying the endpoint is called by an admin modal that renders the response body. It is not, for these paths. The modal posts through `$.post` and renders the body only from its success callback, which does not fire on a non-2xx response, and the admin JavaScript registers no global `ajaxError` handler — so none of the failure bodies reach a user through the shipped UI. The justification is the endpoint's HTTP contract, which its own `400` and `422` paths already establish, not the behavior of one consumer.
...
The real defect is narrower: the action answers its other two failures with a status plus a short text body but lets a missing record fall through to the framework's HTML error page. So the rescue is added for response-format consistency, keeping the `404` and reusing `camaleon_cms.admin.users.message.error` — the key `set_user` already uses, so no new translations.

**Convert all three failure renders to `render plain:`.** Making the new rescue consistent with its siblings raised the question of which direction to converge. `render inline:` compiles its argument as an ERB template, which buys nothing for a fixed message and is a template-injection sink on the `422` path, whose body is `@user.errors.full_messages.join(', ')` — assembled from validation messages, which a model or plugin hook can populate with user-influenced text. Converging on `plain:` also completes a migration the project already started in `67a43f1b` ("Change some render calls from :inline to :plain"), which touched this file but changed only a comment typo, leaving these three renders behind. The bodies are byte-identical; the media type changes from `text/html` to `text/plain`.
