...
- **Tests (new):** member-route parameter-confusion coverage and unresolvable-target coverage under `spec/requests/admin/users_controller/`.
- **Application code:** one rescue clause added to `updated_ajax` in `app/controllers/camaleon_cms/admin/users_controller.rb`. No new translation keys.
- **Behavior:** an unresolvable target on `PATCH /admin/users/:user_id/updated_ajax` keeps its `404` status but gains a plain-text body in place of the framework's HTML error page. Everything else in this change is documentation plus regression coverage of already-shipped behavior.
