...
class RenameColumnPosts < ActiveRecord::Migration
  def change
    remove_column "#{PluginRoutes.system_info["db_prefix"]}posts", :comment_count
    add_column "#{PluginRoutes.system_info["db_prefix"]}posts", :post_order, :integer, default: 0
    add_column "#{PluginRoutes.system_info["db_prefix"]}posts", :taxonomy_id, :integer, default: nil, index: true
